                                    ...
                                    @foreach ($comment->attachments() as $attachment)
                                        <div id="attachment-{{ $attachment->id() }}" class="attachment-box">
                                            ...
                                            <div class="attachment-box-preview">
                                                <div class="delete-attachment-link">
                                                    <a href="javascript:void(0)" class="ajax-delete"
                                                       data-href="{{ route("project.{$controller}.ajax_delete_comment_attachment", [$selectedProjectId, $entityId, $comment->commentId()->id(), $attachment->id()]) }}"
                                                ...
                                                @if ($attachment->type()->isImage())
                                                    <a href="{{ route('project.attachment', [$attachment->id(), $attachment->name()]) }}" rel ="lightbox" data-title="{{ $attachment->name() }}"
                                                       data-lightbox="preview">
                                                        <img src="/thumbs/{{ thumb_url($attachment->id()) }}" width="140" height="100">
                                                    </a>
                                                @else
                                                    <a href="{{ route('project.attachment.download', [$attachment->id(), $attachment->name()]) }}" class="no-preview">
                                            ...
                                            <div class="attachment-box-controls">
                                                {!! Html::linkRoute('project.attachment.download', $attachment->name(), [$attachment->id(), $attachment->name()]) !!}
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
